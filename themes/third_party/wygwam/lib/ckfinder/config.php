...
if ($authorized = (isset($_GET['id']) && isset($_SESSION['wygwam_'.$_GET['id']])))
{
	$sess =& $_SESSION['wygwam_'.$_GET['id']];

	$wygconfig = array(
		'url'           => $sess['u'],
		'server_path'   => $sess['p'],
		'allowed_types' => $sess['t'],
		'max_size'      => $sess['s'],
		'max_height'    => $sess['h'],
		'max_width'     => $sess['w']
	);
...
*/
$config['Images'] = Array(
		'maxWidth' => $wygconfig['max_width'],
		'maxHeight' => $wygconfig['max_height'],
		'quality' => 80);
...
if ($wygconfig['allowed_types'] == 'all')
{
	$config['ResourceType'][] = Array(
