...
if(mobile==0){
	...
	/*
	*Prettify JS
	*src: http://code.google.com/p/google-code-prettify/
	*/
	...
	* src: https://github.com/carhartl/jquery-cookie
	*
	*/
	(function($) {
...
(function(window,undefined ) {
	var sitescripts = {
		ui:{
			...
			skipheader: function(){
				var header_height = $("#gl-header_pri").height();
				$(window).stop().scrollTo(header_height+1,800);
			},
			scrolltop: function(){
				var canvas_height = $(window).height();
				$("#gl-toplink").removeAttr("href");
				$("#gl-toplink").click(function(){
					$(window).stop().scrollTo(0,canvas_height);
				});	
			},
			navhover: function(){
			...
			iemessage: function(){
				...
				var iemessage = 
					...
					"<p>As I've decided to use new and shiny web&ndash;design techniques on my site, I've served you up a more basic set of styles (pretty much the same ones I set for mobile devices). This is so I'm not hampered by IE's poor support for modern web standards. You can of course, <a href='http://www.browserchoice.eu' title='Visit browser choice for the full selection of popular browsers'>upgrade or change your browser</a> if you wish but if not, feel free to enjoy this simplified site.</p>"+
					"</div>"
		...
		modules:{
			...
			tweetbox: function(){
				...
				tweetbox_height = tweetbox_width/3;
		...
		bespokepages:{
			...
			styleguide: function(){
				...
				function viewsource(){
					var $container = $(".canviewsource");
					var code = "";
					var viewablecode = 
						"<pre class='viewable_code'>"+
						"</pre>"
					$container.each(function(){
						var $this = $(this);
						
						//$this.wrapInner(viewablecode);
...
$(document).ready(function() {
	...
	//section specific
		...
		var section=$("html").attr("id");
		var subpage_classstack=$("html").attr("class").split(/\s/);
		var subpage = subpage_classstack[0];
		
		if(section=="pg-contact"){
