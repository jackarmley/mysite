 ...
 * @author		Solspace DevTeam
 * @link		http://solspace.com/docs/
 * @filesource 	./system/bridge/lib/addon_builder/parser.addon_builder.php
 */
...
if (APP_VER < 2.0)
{
	...
	eval('class Addon_builder_parser_base extends Template { }');
}
else
{
	require_once PATH_THIRD . 'bridge/lib/addon_builder/addon_builder.php';
	eval('class Addon_builder_parser_base extends EE_Template { }');
}

class Addon_builder_parser extends Addon_builder_parser_base
{ 
	// --------------------------------------------------------------------
	...
	{   
		...
		if ($this->parse_php == TRUE && $this->php_parse_location == 'input')
		{
		...
		/** -------------------------------------*/
   		...
   		if (strpos($this->template, LD.'assign_variable:') !== FALSE && preg_match_all("/".LD."assign_variable:(.+?)=([\"\'])([^\\2]*?)\\2".RD."/i", $this->template, $matches))
   		{	
		...
		if ($this->parse_php == TRUE && $this->php_parse_location == 'output')
		{
    ...
    public function class_handler()
    {
        ...
        { 
			...
			if (in_array($lower_class_name, $this->plugins) && strpos($this->tag_data[$i]['block'], LD.'exp:') !== false)
			{
			...
			if (in_array($lower_class_name, $this->modules) && ! isset($this->module_data[$class_name]))
			{
			...
			if ((in_array($lower_class_name, $this->modules) && ! isset($this->module_data[$class_name])) OR ! method_exists($EE, $method))
			{
		...
		if (($map = directory_map(PATH_THIRD)) !== FALSE)
		{
			foreach ($map as $pkg_name => $files)
			{
				...
				foreach ($files as $file)
				{
					...
					// Module
					if (strncasecmp($file, 'mod.', 4) == 0 && strlen($file) >= 9) // strlen('mod.a.php');
					{
					...
					// Plugin
					elseif (strncasecmp($file, 'pi.', 3) == 0 && strlen($file) >= 8) // strlen('pi.a.php');
					{							
    ...
    public function fetch_modules()
    {
        ...
        foreach(array(PATH_MOD) as $directory)
		{
			...
			{ 
				...
				{
					if ( is_dir($directory.$file) && ! preg_match("/[^a-z\_0-9]/", $file))
					{
						$this->modules[] = $file;
					}
				}
