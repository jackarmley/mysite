...
class Playa_Helper {
	 ...
	 * Module Tag Alias
	 */
	function mod_tag_alias($params, $tagdata, $func)
	{
		// make sure the module is loaded
		if (! class_exists('Playa'))
		{
			require_once PATH_THIRD.'playa/mod.playa.php';
		}

		// -------------------------------------------
		//  Remember the old template vars
		// -------------------------------------------

		$old_params = $this->EE->TMPL->tagparams;
		$old_tagdata = $this->EE->TMPL->tagdata;
		$old_var_single = $this->EE->TMPL->var_single;
		$old_var_pair = $this->EE->TMPL->var_pair;
		$old_site_ids = $this->EE->TMPL->site_ids;

		// -------------------------------------------
		//  Call the module's tag countepart
		// -------------------------------------------
		...
		// -------------------------------------------
		//  Restore the old template vars
		// -------------------------------------------

		$this->EE->TMPL->tagparams = $old_params;
		$this->EE->TMPL->tagdata = $old_tagdata;
		$this->EE->TMPL->var_single = $old_var_single;
		$this->EE->TMPL->var_pair = $old_var_pair;
		$this->EE->TMPL->site_ids = $old_site_ids;
